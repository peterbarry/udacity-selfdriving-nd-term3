...
int main() {
  ...
  //cout << "Updating the D and Ddot to relative lane position";
  //cout << X_train;
  for (int x; x < X_train.size(); x++) {
    //cout << "orig:" << X_train[x][1] << "  " << X_train[x][3] << endl;

    X_train[x][1] = fmod(X_train[x][1] , 4.0) ;  // size of lane
    X_train[x][3] = fmod(X_train[x][3] , 4.0) ; // size of lane

  //  cout << "upda:" << X_train[x][1] << "  " << X_train[x][3] << endl;
